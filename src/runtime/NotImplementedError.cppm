module;
#include "core.hpp"
#include "memory/allocate.hpp"

export module py.runtime:not_implemented_error;
import :exception;
import :object;
import py.memory;
import std;

export namespace py {
struct Number;
class PyTuple;
class PyString;
class PyDict;
class BaseException;
class PyType;

class NotImplementedError : public Exception
{
	friend class ::Heap;
	friend class py::detail::Allocator;
	friend BaseException *make_not_implemented_error(std::string &&);

  private:
	NotImplementedError(PyType *);

	NotImplementedError(PyTuple *args);

	static NotImplementedError *create(PyTuple *args);

  public:
	static PyResult<PyObject *> __new__(const PyType *type, PyTuple *args, PyDict *kwargs);

	static std::function<std::unique_ptr<TypePrototype>()> type_factory();

	PyType *static_type() const override;
};

// Defined in NotImplementedError.cpp. Keeping the PyString/PyTuple construction and the
// heap allocation out of the interface means this partition no longer has
// to materialise those types just to declare one exception class.
BaseException *make_not_implemented_error(std::string &&message);

template<typename... Args>
inline BaseException *not_implemented_error(const std::string &message, Args &&...args)
{
	return make_not_implemented_error(std::vformat(message, std::make_format_args(args...)));
}

}// namespace py
