...
export module py.runtime:klass;
import :arg_parser;
import :dict;
import :type_error;
import :integer;
...
namespace detail {
// A local copy of utilities.hpp's trait: that header pulls spdlog (and with it
// 262 libstdc++ headers), which cannot enter a module GMF.
...
template<typename T> struct klass
{
				...
				if constexpr (std::is_convertible_v<MemberType, PyObject *>) {
					...
					if constexpr (std::numeric_limits<MemberType>::is_signed) {
						return PyInteger::create(
							static_cast<std::int64_t>(static_cast<T *>(self)->*member));
					} else {
						const auto value =
							static_cast<std::uint64_t>(static_cast<T *>(self)->*member);
						if (value > std::numeric_limits<std::int64_t>::max()) { TODO(); }
						return PyInteger::create(static_cast<std::int64_t>(value));
					}
				} else {
		...
		type->add_method(MethodDefinition{
			...
			.method = [F, name](
				...
				const std::size_t arg_count =
					(args ? args->size() : 0) + (kwargs ? kwargs->size() : 0);
				if (arg_count) {
	...
	PyType *finalize()
	{
		...
		::detail::log_trace(
			std::format("Added type@{} with name {}", (void *)type_, type_->name()).c_str());
