...
export module py.runtime:runtime_error;
...
class RuntimeError : public Exception
{
	...
	friend BaseException *make_runtime_error(std::string &&);
...
// Defined in RuntimeError.cpp. Keeping the PyString/PyTuple construction and the
...
BaseException *make_runtime_error(std::string &&message);

template<typename... Args>
inline BaseException *runtime_error(const std::string &message, Args &&...args)
{
	return make_runtime_error(std::vformat(message, std::make_format_args(args...)));
}
