...
export module py.runtime:object;
...
class MethodFlags
{
  ...
  public:
	...
	bool is_set(Flag f) const { return m_flags.test(static_cast<std::size_t>(f)); }
...
using InitSlotFunctionType = std::function<PyResult<std::int32_t>(PyObject *, PyTuple *, PyDict *)>;
...
using RepeatSequenceSlotFunctionType = std::function<PyResult<PyObject *>(PyObject *, std::size_t)>;
using GetItemSequenceSlotFunctionType =
	std::function<PyResult<PyObject *>(PyObject *, std::int64_t)>;
using SetItemSequenceSlotFunctionType =
	std::function<PyResult<std::monostate>(PyObject *, std::int64_t, PyObject *)>;
using DelItemSequenceSlotFunctionType =
	std::function<PyResult<std::monostate>(PyObject *, std::int64_t)>;

using LenSlotFunctionType = std::function<PyResult<std::size_t>(const PyObject *)>;
...
using HashSlotFunctionType = std::function<PyResult<std::int64_t>(const PyObject *)>;
...
struct PyBuffer
{
	...
	std::vector<std::int64_t> suboffsets;
	void *internal;
...
std::size_t get_address(const std::variant<std::function<T(U...)>, PyObject *> &f)
{
	...
	if (std::holds_alternative<std::function<T(U...)>>(f)) {
		...
		return std::bit_cast<std::size_t>(*fn_ptr);
	} else {
		// FIXME: is it valid to take this path? Is there use case?
		return std::bit_cast<std::size_t>(std::get<PyObject *>(f));
	}
}
...
class PySequenceWrapper
{
  ...
  public:
	...
	PyResult<PyObject *> getitem(std::int64_t);// sq_item
	PyResult<std::monostate> setitem(std::int64_t, PyObject *);// sq_ass_item
	PyResult<std::monostate> delitem(std::int64_t);// sq_ass_item
...
class PyObject : public Cell
{
  ...
  public:
	...
	PyResult<std::int64_t> hash() const;
	...
	PyResult<std::int32_t> init(PyTuple *args, PyDict *kwargs);

	PyResult<PyObject *> getitem(PyObject *key);
	PyResult<PyObject *> getitem(std::size_t index);
...
template<> PyResult<PyObject *> PyObject::from(const std::int64_t &value);
...
std::unique_ptr<TypePrototype> TypePrototype::create(std::string_view name, Args &&...args)
{
	...
	type_prototype->__alloc__ = [](PyType *t) -> PyResult<PyObject *> {
		const auto nslots = py::detail::slot_count(t);
		// Allocator, not std::construct_at: the concrete types keep their
		// constructors private and befriend ::Heap and detail::Allocator, and
		// construct_at cannot reach a private constructor the way the placement
		// new inside Allocator can.
		auto *obj =
			detail::Allocator::allocate_with_extra_bytes<Type>(nslots * sizeof(PyObject *), t);
		// FIXME: this should call Err(memory_error(sizeof(Type))), but would cause circular
		// dependency
		if (!obj) { TODO(); }
		return Ok(obj);
	};
	...
	if constexpr (concepts::HasInit<Type>) {
		type_prototype->__init__ =
			+[](PyObject *self, PyTuple *args, PyDict *kwargs) -> PyResult<std::int32_t> {
	...
	if constexpr (concepts::HasHash<Type>) {
		type_prototype->__hash__ = +[](const PyObject *self) -> PyResult<std::int64_t> {
	...
	if constexpr (concepts::HasLength<Type>) {
		detail::ensure_mapping_proto(*type_prototype).__len__ =
			+[](const PyObject *self) -> PyResult<std::size_t> {
	...
	if constexpr (concepts::HasSequenceGetItem<Type>) {
		detail::ensure_sequence_proto(*type_prototype).__getitem__ =
			+[](PyObject *self, std::int64_t index) -> PyResult<PyObject *> {
	...
	if constexpr (concepts::HasSequenceSetItem<Type>) {
		detail::ensure_sequence_proto(*type_prototype).__setitem__ =
			+[](PyObject *self, std::int64_t index, PyObject *value) -> PyResult<std::monostate> {
	...
	if constexpr (concepts::HasSequenceDelItem<Type>) {
		detail::ensure_sequence_proto(*type_prototype).__delitem__ =
			+[](PyObject *self, std::int64_t index) -> PyResult<std::monostate> {
	...
	if constexpr (std::is_base_of_v<PySequence, Type> && concepts::HasRepeat<Type>) {
		detail::ensure_sequence_proto(*type_prototype).__repeat__ =
			+[](const PyObject *self, std::size_t count) -> PyResult<PyObject *> {
