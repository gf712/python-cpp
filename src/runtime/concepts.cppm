...
export module py.runtime:concepts;
...
namespace concepts {
	...
	template<typename T>
	concept HasSequenceGetItem = requires(T *obj, std::int64_t index) { obj->__getitem__(index); };
	template<typename T>
	concept HasSequenceSetItem =
		requires(T *obj, std::int64_t index, PyObject *value) { obj->__setitem__(index, value); };
	template<typename T>
	concept HasSequenceDelItem = requires(T *obj, std::int64_t index) { obj->__delitem__(index); };
