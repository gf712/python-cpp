...
export module py.runtime:value;
import std;

export class Interpreter;

// std::formatter rather than std::formatter: spdlog is built with
// SPDLOG_USE_STD_FORMAT, so nothing here needs spdlog/fmt/fmt.h - which would
// otherwise pull 231 libstdc++ headers into this leaf partition's BMI.
template<> struct std::formatter<mpz_class> : std::formatter<std::string>
{
	template<typename FormatContext> auto format(const mpz_class &number, FormatContext &ctx) const
	{
