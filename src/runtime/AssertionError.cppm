module;
#include "memory/allocate.hpp"

export module py.runtime:assertion_error;
import :exception;
import :object;
import py.memory;
import std;

export namespace py {
struct Number;
class PyTuple;
class PyString;
class PyDict;
class BaseException;
class PyType;

class AssertionError : public Exception
{
	friend class ::Heap;
	friend class py::detail::Allocator;
	friend BaseException *make_assertion_error(std::string &&);

  private:
	AssertionError(PyType *t);

	AssertionError(PyTuple *args);

	static PyResult<AssertionError *> create(PyTuple *args);

  public:
	static std::function<std::unique_ptr<TypePrototype>()> type_factory();

	static PyResult<PyObject *> __new__(const PyType *type, PyTuple *args, PyDict *kwargs);

	PyType *static_type() const override;

	static PyType *this_type();

	std::string to_string() const override;
};

// Defined in AssertionError.cpp. Keeping the PyString/PyTuple construction and the
// heap allocation out of the interface means this partition no longer has
// to materialise those types just to declare one exception class.
BaseException *make_assertion_error(std::string &&message);

template<typename... Args>
inline BaseException *assertion_error(const std::string &message, Args &&...args)
{
	return make_assertion_error(std::vformat(message, std::make_format_args(args...)));
}
}// namespace py
