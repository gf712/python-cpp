...
export module py.lexer;
import std;

constexpr std::size_t tab_size = 8;
...
export class Lexer
{
  ...
  private:
	...
	Position m_position;
	std::vector<std::size_t> m_indent_values;
	...
	std::stack<std::size_t> m_fstring_paren_level;
  ...
  public:
	...
	// 		::detail::log_error(std::format("Failed to parse program at position {}",
	// m_position).c_str()); 		std::abort();
	// 	}
	// 	ASSERT_NOT_REACHED();
	// }

	std::optional<Token> peek_token(std::size_t positions)
	{
	...
	{
		const auto size =
			static_cast<std::size_t>(end.pointer_to_program - start.pointer_to_program);
  ...
  private:
	...
	std::optional<std::size_t> comment_start() const;
	...
	std::tuple<std::size_t, std::size_t> compute_indent_level();
	...
	std::optional<std::size_t> hex_number(std::size_t) const;
	std::optional<std::size_t> bin_number(std::size_t) const;
	std::optional<std::size_t> oct_number(std::size_t) const;
	std::optional<std::size_t> dec_number(std::size_t) const;
	std::optional<std::size_t> int_number(std::size_t) const;
	std::optional<std::size_t> exp_number(std::size_t) const;
	std::optional<std::size_t> point_float_number(std::size_t) const;
	std::optional<std::size_t> exp_float_number(std::size_t) const;
	std::optional<std::size_t> float_number(std::size_t) const;
	std::optional<std::size_t> imag_number(std::size_t) const;

	bool single_quote_string(std::size_t);
	bool double_quote_string(std::size_t);
	bool single_triple_quote_string(std::size_t);
	bool double_triple_quote_string(std::size_t);

	template<typename FunctorType>
	std::optional<std::size_t> parse_digits(std::size_t n, FunctorType &&f) const;

	char peek(std::size_t i) const;

	bool peek(std::string_view pattern) const;

	template<typename ConditionType> std::size_t advance_while(ConditionType &&condition)
	{
	...
	void advance(const std::size_t positions);
	...
	void increment_column_position(std::size_t idx);
