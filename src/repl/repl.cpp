...
namespace {
int run_and_execute_script(int argc, char **argv)
{
	size_t arg_idx{ 1 };
	...
	p.parse();
	auto bytecode =
		BytecodeGenerator::compile(p.module(), argv_vector, compiler::OptimizationLevel::None);
	vm.clear();
...
int main(int argc, char **argv)
{
	if (argc > 1) {
		...
		do {
			...
			} else if (strcmp(argv[parsed_args], "--trace") == 0) {
				spdlog::set_level(spdlog::level::trace);
			} else {
				filename = argv[parsed_args];
			}
		} while (++parsed_args < argc);
		if (filename) { return run_and_execute_script(argc, argv); }
	}
