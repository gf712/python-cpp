...
// std::vector<std::byte> rather than py::Bytes: the AST deliberately does not
// depend on the runtime's value representation, so a bytes literal is carried
// as raw bytes and converted in codegen.
// Python's `...`. std::monostate already stands for None, so Ellipsis needs
// its own alternative.
struct EllipsisType
{
	bool operator==(const EllipsisType &) const = default;
};
...
class Constant : public ASTNode
{
  ...
  public:
	...
	Constant(std::int64_t value, SourceLocation source_location);
	...
	Constant(Bytes value, SourceLocation source_location);
	Constant(const Literal &, SourceLocation source_location);
