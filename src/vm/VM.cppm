...
export using Registers = std::vector<py::Value>;
...
export struct StackFrame : NonCopyable
{
  private:
	StackFrame() = default;

	StackFrame(std::size_t register_count,
	...
	, NonMoveable
{
	...
	static constexpr std::size_t kStackSize = 10'000;
  ...
  public:
	...
	const py::Value &reg(std::size_t idx) const
	{
		auto r = registers();
		ASSERT(r.has_value());
		ASSERT(idx < r->get().size());
	...
	py::Value &stack_local(std::size_t idx)
	{
	...
	const py::Value &stack_local(std::size_t idx) const
	{
		auto local = stack_locals();
		ASSERT(!local.empty());
		ASSERT(idx < local.size());
	...
	[[nodiscard]] std::unique_ptr<StackFrame> setup_call_stack(std::size_t register_count,
		std::size_t locals_count,
		std::size_t stack_size);
	...
	[[nodiscard]] std::unique_ptr<StackFrame>
		push_frame(std::size_t register_count, std::size_t locals_count, std::size_t stack_size);
