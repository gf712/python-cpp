...
class SetupExceptionHandling final : public Instruction
{
  ...
  public:
	...
	SetupExceptionHandling(std::uint32_t offset) : m_offset(offset) {}

	std::string to_string() const final
	{
		const std::string position =
			m_offset.has_value() ? std::to_string(*m_offset) : "offset not evaluated";
		return std::format("SETUP_EXC_HANDLE position: {}", position);
	}
	...
	std::uint8_t id() const final { return SETUP_EXCEPTION_HANDLING; }
};
