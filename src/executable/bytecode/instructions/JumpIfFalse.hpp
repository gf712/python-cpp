...
class JumpIfFalse final : public Instruction
{
  ...
  public:
	...
	JumpIfFalse(Register test_register, std::int32_t offset)
	...
	std::string to_string() const final
	{
		...
		return std::format("JUMP_IF_FALSE   r{:<3} position: {}", m_test_register, position);
	}
	...
	std::uint8_t id() const final { return JUMP_IF_FALSE; }
};
