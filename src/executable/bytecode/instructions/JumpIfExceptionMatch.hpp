...
class JumpIfExceptionMatch final : public Instruction
{
  ...
  public:
	...
	{}
	JumpIfExceptionMatch(Register exception_type_reg, std::uint32_t offset)
	...
	std::string to_string() const final
	{
		...
		return std::format("JUMP_IF_EXC     r{} position: {}", m_exception_type_reg, position);
	}
	...
	std::uint8_t id() const final { return JUMP_IF_EXCEPTION_MATCH; }
};
