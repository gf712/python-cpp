...
class JumpIfTrueOrPop final : public Instruction
{
  ...
  public:
	...
	JumpIfTrueOrPop(Register test_register, Register result_register, std::int32_t offset)
	...
	std::string to_string() const final
	{
		...
		return std::format("JUMP_IF_TRUE_OR_POP r{:<3} r{:<3} position: {}",
			m_test_register,
			m_result_register,
			position);
	}
	...
	std::uint8_t id() const final { return JUMP_IF_TRUE_OR_POP; }
};
