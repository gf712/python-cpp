...
export module py.codegen:bytecode_generator;
import py.ast;
import py.runtime;
import std;
import :variables_resolver;

export py::Value to_runtime_value(const ast::Literal &literal);
...
struct FunctionInfo
{
	...
	FunctionInfo(std::size_t, FunctionBlock &, BytecodeGenerator *);
};
...
class BytecodeStaticValue : public ast::Value
{
  ...
  public:
	BytecodeStaticValue(std::size_t index) : ast::Value("constant"), m_index(index) {}
...
class BytecodeNameValue : public ast::Value
{
  ...
  public:
	BytecodeNameValue(const std::string &name, std::size_t index) : ast::Value(name), m_index(index)
	{}
...
class BytecodeGenerator : public ast::CodeGenerator
{
	...
	{
		ScopedClearExceptionBeforeReturn(BytecodeGenerator &generator_, std::size_t function_id_)
  ...
  public:
	static constexpr std::size_t start_register = 1;
	static constexpr std::size_t start_stack_index = 0;
  ...
  private:
	...
	std::vector<std::size_t> m_frame_register_count;
	std::vector<std::size_t> m_frame_stack_value_count;
	std::vector<std::size_t> m_frame_free_var_count;

	std::unordered_map<std::string, std::reference_wrapper<std::size_t>> m_function_free_var_count;
	...
	std::set<std::size_t> m_clear_exception_before_return_functions;
	std::unordered_map<std::size_t, std::vector<std::function<void(bool)>>> m_return_transform;
	std::unordered_map<std::size_t, std::size_t> m_current_exception_depth;
  ...
  private:
	...
	const InstructionVector &function(std::size_t idx) const
	{
		ASSERT(idx < m_functions.functions.size());
		return std::next(m_functions.functions.begin(), idx)->blocks;
	}

	std::shared_ptr<Label> make_label(const std::string &name, std::size_t function_id);

	const std::vector<std::shared_ptr<Label>> &labels() const { return m_labels; }

	void bind(const std::shared_ptr<Label> &label);

	std::size_t register_count() const { return m_frame_register_count.back(); }
	std::size_t stack_variable_count() const { return m_frame_stack_value_count.back(); }
	std::size_t free_variable_count() const { return m_frame_free_var_count.back(); }
	...
	InstructionVector *allocate_block(std::size_t);
	...
	void exit_function(std::size_t function_id);

	void store_name(const std::string &, BytecodeValue *);
	BytecodeValue *load_var(const std::string &);
	void delete_var(const std::string &);

	BytecodeStaticValue *load_const(const py::Value &, std::size_t);
	BytecodeNameValue *load_name(const std::string &, std::size_t);
...
#undef __AST_NODE_TYPE
	...
	BytecodeValue *generate(const ast::ASTNode *node, std::size_t function_id)
	{
