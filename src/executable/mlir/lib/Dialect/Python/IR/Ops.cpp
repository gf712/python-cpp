...
#include "Python/Dialect.cpp.inc"
...
namespace py {
	void PythonDialect::initialize()
	{
		addOperations<
#define GET_OP_LIST
#include "Python/Ops.cpp.inc"
			>();

		addTypes<
#define GET_TYPEDEF_LIST
#include "Python/PythonTypes.cpp.inc"
			>();
	}
		...
		std::vector<std::byte> bytes)
	{
		auto byte_array_attr = mlir::DenseIntElementsAttr::get(
			...
			mlir::ArrayRef<unsigned char>{ bit_cast<unsigned char *>(bytes.data()), bytes.size() });
