...
mlir::Location loc(mlir::OpBuilder &builder, const SourceLocation &loc)
{
	return mlir::FileLineColLoc::get(
		builder.getStringAttr("test.py"), loc.start.row, loc.start.column);
}
...
mlir::py::ConstantOp
	load_const(mlir::OpBuilder &builder, py::BigIntType integer, SourceLocation source_location)
{
	auto value = [&]() {
		if (integer == 0) {
	...
	auto op = builder.create<mlir::py::ConstantOp>(loc(builder, source_location), value);
	return op;
}

mlir::py::ConstantOp
	load_const(mlir::OpBuilder &builder, std::string_view str, SourceLocation source_location)
{
	return builder.create<mlir::py::ConstantOp>(
		loc(builder, source_location), builder.getStringAttr(str));
}
...
bool MLIRGenerator::compile(std::shared_ptr<ast::Module> m, Context &ctx)
{
	...
	const SourceLocation &location)
{
	...
	case VariablesResolver::Visibility::NAME: {
		m_context.builder().create<mlir::py::StoreNameOp>(
			loc(m_context.builder(), location), m_context->pyobject_type(), name, value->value);
	}
	...
	const SourceLocation &location)
{
	...
	case VariablesResolver::Visibility::NAME: {
		return new_value(m_context.builder().create<mlir::py::LoadNameOp>(
			loc(m_context.builder(), location), m_context->pyobject_type(), name));
	}
	case VariablesResolver::Visibility::LOCAL: {
	...
	case VariablesResolver::Visibility::IMPLICIT_GLOBAL: {
		...
		if (&m_scope.front() == &scope()) {
			m_context.builder().create<mlir::py::DeleteNameOp>(
				loc(m_context.builder(), location), name);
		} else {
	...
	const SourceLocation &source_location)
{
	if (auto ast_name = as<ast::Name>(target)) {
		...
		m_context.builder().create<mlir::py::StoreSubscriptOp>(
			loc(m_context.builder(), source_location), value, index, src->value);
		...
		auto unpack_sequence = m_context.builder().create<mlir::py::UnpackSequenceOp>(
			loc(m_context.builder(), source_location), unpacked_types, src->value);
		...
		m_context.builder().create<mlir::py::StoreAttributeOp>(
			loc(m_context.builder(), source_location), obj, name, src->value);
	} else {
...
ast::Value *MLIRGenerator::visit(const ast::Assert *node)
{
	...
	auto cond = m_context.builder().create<mlir::py::CastToBoolOp>(
		loc(m_context.builder(), node->test()->source_location()),
	...
	auto assertion_error = [this, node]() {
		auto assertion_error_fn = m_context.builder().create<mlir::py::LoadAssertionError>(
			loc(m_context.builder(), node->source_location()), m_context->pyobject_type());
		if (node->msg()) {
	...
	m_context.builder().create<mlir::py::RaiseOp>(
		loc(m_context.builder(), node->source_location()), assertion_error);
...
ast::Value *MLIRGenerator::visit(const ast::AugAssign *node)
{
	...
	if (auto named_target = as<ast::Name>(node->target())) {
		...
		m_context.builder().create<mlir::py::StoreAttributeOp>(
			loc(m_context.builder(), node->target()->source_location()),
			*target_value,
			name,
			target->value);
...
ast::Value *MLIRGenerator::visit(const ast::Break *node)
{
	...
	m_context.builder().create<mlir::cf::BranchOp>(
		loc(m_context.builder(), node->source_location()), b);
			...
			for (const auto &kwarg : node->keywords()) {
				...
				if (kwarg_requires_expansion.emplace_back(is_kwargs_expansion(kwarg))) {
					...
					keys.push_back(new_value(m_context.builder().create<mlir::py::ConstantOp>(
						loc(m_context.builder(), kwarg->source_location()),
...
ast::Value *MLIRGenerator::visit(const ast::ClassDefinition *node)
{
	...
	std::vector<std::string> captures;
	{
		...
		store_name("__qualname__",
			new_value(load_const(m_context.builder(), node->name(), node->source_location())),
			node->source_location());

		for (const auto &el : node->body()) { el->codegen(this); }

		m_context.builder().create<mlir::cf::BranchOp>(
			loc(m_context.builder(), node->body().back()->source_location()), return_block);
	...
	return std::visit(
		overloaded{
						...
						[this, node](double value) {
							mlir::py::ConstantOp op =
								m_context.builder().create<mlir::py::ConstantOp>(
									loc(m_context.builder(), node->source_location()), value);
							return new_value(op);
						},
						[this, node](const py::BigIntType &int_value) {
							return new_value(load_const(
								m_context.builder(), int_value, node->source_location()));
						},
					},
					number.value);
			},
			...
			[this, node](const py::String &s) -> ast::Value * {
				return new_value(load_const(m_context.builder(), s.s, node->source_location()));
			},
			[this, node](const py::Bytes &b) -> ast::Value * {
				mlir::py::ConstantOp op = m_context.builder().create<mlir::py::ConstantOp>(
					loc(m_context.builder(), node->source_location()), b.b);
				return new_value(op);
			},
			[](auto) -> ast::Value * {
				TODO();
...
ast::Value *MLIRGenerator::visit(const ast::Dict *node)
{
	...
	auto none = new_value(m_context.builder().create<mlir::py::ConstantOp>(
		loc(m_context.builder(), node->source_location()), m_context.builder().getNoneType()));
	...
	return build_comprehension(
		...
		[this, node](MLIRValue *container) {
			...
			m_context.builder().create<mlir::py::DictAddOp>(
				loc(m_context.builder(), node->key()->source_location()),
				container->value,
				key,
				value);
		},
	...
	return build_comprehension(
		...
		[this, node](MLIRValue *) {
			...
			m_context.builder().create<mlir::py::YieldOp>(
				loc(m_context.builder(), node->elt()->source_location()),
				m_context->pyobject_type(),
				result);
		},
		node->generators(),
...
ast::Value *MLIRGenerator::visit(const ast::If *node)
{
	...
	m_context.builder().setInsertionPointToEnd(cond.getOperation()->getBlock());
	m_context.builder().create<mlir::cf::CondBranchOp>(
		loc(m_context.builder(), node->test()->source_location()), cond, if_block, orelse_block);
	...
	if (m_context.builder().getBlock()->empty()
		...
		m_context.builder().create<mlir::cf::BranchOp>(
			loc(m_context.builder(), node->body().back()->source_location()), continuation);
	}
	if (!node->orelse().empty()) {
		...
		if (m_context.builder().getBlock()->empty()
			|| !m_context.builder().getBlock()->back().hasTrait<mlir::OpTrait::IsTerminator>()) {
			m_context.builder().create<mlir::cf::BranchOp>(
				loc(m_context.builder(), node->orelse().back()->source_location()), continuation);
		}
	} else {
...
ast::Value *MLIRGenerator::visit(const ast::IfExpr *node)
{
	...
	auto orelse_block = m_context.builder().createBlock(parent);
	auto continuation = m_context.builder().createBlock(parent);
	continuation->addArgument(
		m_context->pyobject_type(), loc(m_context.builder(), node->source_location()));
	...
	m_context.builder().create<mlir::cf::BranchOp>(
		loc(m_context.builder(), node->body()->source_location()),
		continuation,
	...
	m_context.builder().create<mlir::cf::BranchOp>(
		loc(m_context.builder(), node->orelse()->source_location()),
		continuation,
	...
	for (const auto &n : node->names()) {
		if (n.name == "*") {
			m_context.builder().create<mlir::py::ImportAllOp>(
				loc(m_context.builder(), node->source_location()), module);
		} else {
	...
	return build_comprehension(
		...
		[this, node](MLIRValue *container) {
			...
			m_context.builder().create<mlir::py::ListAppendOp>(
				loc(m_context.builder(), node->elt()->source_location()), container->value, result);
		},
		node->generators(),
		node->source_location());
}
...
ast::Value *MLIRGenerator::visit(const ast::Raise *node)
{
	if (node->cause()) {
		...
		m_context.builder().create<mlir::py::RaiseOp>(
			loc(m_context.builder(), node->source_location()), exception, cause);
	} else if (node->exception()) {
		auto exception = static_cast<const MLIRValue &>(*node->exception()->codegen(this)).value;
		m_context.builder().create<mlir::py::RaiseOp>(
			loc(m_context.builder(), node->source_location()), exception);
	} else {
		m_context.builder().create<mlir::py::RaiseOp>(
			loc(m_context.builder(), node->source_location()));
	}
	...
	const SourceLocation &source_location)
{
	...
	auto f =
		m_context.builder().create<mlir::func::FuncOp>(loc(m_context.builder(), source_location),
			mangled_name,
			func_type,
	...
	std::vector<std::string> captures;
	{
		...
		auto next_generator = [this](mlir::Value iterable,
			...
			auto for_loop = m_context.builder().create<mlir::py::ForLoopOp>(
				loc(m_context.builder(), generator->source_location()), iterable);
			m_context.builder().create<mlir::py::ControlFlowYield>(
			...
			// loop body
			{
				...
				if (!generator->ifs().empty()) {
					...
					for (const auto &el :
						...
						auto cond = m_context.builder().create<mlir::py::CastToBoolOp>(
							loc(m_context.builder(), el->source_location()),
							m_context.builder().getI1Type(),
		...
		auto iterable = m_context.builder().create<mlir::py::LoadFastOp>(
			loc(m_context.builder(), first_generator->iter()->source_location()),
			m_context->pyobject_type(),
			".0");
		...
		if (container) {
			...
			auto none = m_context.builder().create<mlir::py::ConstantOp>(
				loc(m_context.builder(), first_generator->target()->source_location()),
				m_context.builder().getNoneType());
	...
	auto fn_obj = m_context.builder().create<mlir::py::MakeFunctionOp>(
		loc(m_context.builder(), source_location),
		m_context->pyobject_type(),
		mangled_name,
		mlir::ValueRange{},
		mlir::ValueRange{},
...
codegen::MLIRGenerator::MLIRValue *MLIRGenerator::build_list(
	const std::vector<codegen::MLIRGenerator::MLIRValue *> &els,
	const std::vector<bool> &requires_expansion,
	const SourceLocation &location)
{
	...
	std::vector<int8_t> requires_expansion_i8;
	requires_expansion_i8.reserve(requires_expansion.size());
	std::transform(requires_expansion.begin(),
		requires_expansion.end(),
		std::back_inserter(requires_expansion_i8),
		[](bool el) -> int8_t { return el; });

	return new_value(
		m_context.builder().create<mlir::py::BuildListOp>(loc(m_context.builder(), location),
			m_context->pyobject_type(),
			elements,
			mlir::DenseI8ArrayAttr::get(&m_context.ctx(), requires_expansion_i8)));
}
	...
	const SourceLocation &location)
{
	...
	return build_dict(keys, values, requires_expansion, location);
}
	...
	const SourceLocation &location)
{
	...
	return new_value(
		m_context.builder().create<mlir::py::BuildDictOp>(loc(m_context.builder(), location),
			m_context->pyobject_type(),
			ks,
			vs,
			mlir::DenseI8ArrayAttr::get(&m_context.ctx(), requires_expansion_i8)));
	...
	const SourceLocation &location)
{
	...
	return new_value(
		m_context.builder().create<mlir::py::BuildTupleOp>(loc(m_context.builder(), location),
			m_context->pyobject_type(),
			elements,
			mlir::DenseI8ArrayAttr::get(&m_context.ctx(), requires_expansion_i8)));
}
	...
	const SourceLocation &location)
{
	...
	return new_value(
		m_context.builder().create<mlir::py::BuildSetOp>(loc(m_context.builder(), location),
			m_context->pyobject_type(),
			std::move(elements),
			mlir::DenseI8ArrayAttr::get(&m_context.ctx(), requires_expansion_i8)));
}
...
void MLIRGenerator::return_value(MLIRValue *value, const SourceLocation &source_location)
{
	...
	if (!scope().finally_blocks.empty()) {
		// m_context.builder().create<mlir::cf::BranchOp>(
		// 	loc(m_context.builder(), node->source_location()), scope().finally_blocks.top());
	...
	m_context.builder().create<mlir::func::ReturnOp>(
		loc(m_context.builder(), source_location), value->value);
}
...
ast::Value *MLIRGenerator::visit(const ast::Subscript *node)
{
	...
	case ast::ContextType::DELETE: {
		m_context.builder().create<mlir::py::DeleteSubscriptOp>(
			loc(m_context.builder(), node->source_location()), value, index);
		return nullptr;
	} break;
...
ast::Value *MLIRGenerator::visit(const ast::Try *node)
{
	...
	auto try_op = m_context.builder().create<mlir::py::TryOp>(
		loc(m_context.builder(), node->source_location()), node->handlers().size());
	...
	scope().finally_blocks.emplace_back([this, node](bool first) {
		...
		m_context.builder().setInsertionPointToEnd(current);
		m_context.builder().create<mlir::cf::BranchOp>(
			loc(m_context.builder(), node->source_location()), final_block);
	...
	for (auto p : llvm::enumerate(llvm::zip(try_op.getHandlers(), node->handlers()))) {
		...
		auto handler_op = m_context.builder().create<mlir::py::TryHandlerScope>(
			loc(m_context.builder(), handler->source_location()));
...
ast::Value *MLIRGenerator::visit(const ast::UnaryExpr *node)
{
	...
	case ast::UnaryOpType::ADD: {
		return new_value(m_context.builder().create<mlir::py::PositiveOp>(
			loc(m_context.builder(), node->source_location()), m_context->pyobject_type(), src));
	} break;
...
ast::Value *MLIRGenerator::visit(const ast::While *node)
{
	...
	if (m_context.builder().getBlock()->empty()
		|| !m_context.builder().getBlock()->back().hasTrait<mlir::OpTrait::IsTerminator>()) {
		m_context.builder().create<mlir::py::ControlFlowYield>(
			loc(m_context.builder(), node->body().back()->source_location()));
	}
...
ast::Value *MLIRGenerator::visit(const ast::Yield *node)
{
	...
	return new_value(m_context.builder().create<mlir::py::YieldOp>(
		loc(m_context.builder(), node->source_location()), m_context->pyobject_type(), value));
}
